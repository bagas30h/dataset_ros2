...
def cil_model():
    ...
    for _ in range(7):  # Remaining 7 layers
        kernel_size = (3, 3)
        stride = (1, 1) if (_ % 2 == 1) else (2, 2)  # Stride 2 for odd layers
        model.add(Conv2D(32, kernel_size=kernel_size, strides=stride, padding='same'))
        model.add(BatchNormalization())
        model.add(Activation('relu'))
    
    # Flatten the output before connecting to FCN layers
    model.add(Flatten())
    ...
    model.add(Dense(128))  # Second FCN layer
    model.add(BatchNormalization())
    model.add(Activation('relu'))
...
# Save the model
model.save('model_mobil_besar_cil.h5')
print('Model saved')
